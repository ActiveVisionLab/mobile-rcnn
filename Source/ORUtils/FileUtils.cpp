...
void SaveImageToFile(const ORUtils::Image<ORUtils::Vector4<unsigned char> > * image, const char* fileName, bool flipVertical)
{
	...
	if (flipVertical)
	{
		for (int y = 0; y < noDims.y; y++) for (int x = 0; x < noDims.x; x++)
		{
			...
			data[locId_dst * 3 + 0] = image->Data(MEMORYDEVICE_CPU)[locId_src].x;
			data[locId_dst * 3 + 1] = image->Data(MEMORYDEVICE_CPU)[locId_src].y;
			data[locId_dst * 3 + 2] = image->Data(MEMORYDEVICE_CPU)[locId_src].z;
		}
	}
	else
	{
		for (int i = 0; i < noDims.x * noDims.y; ++i) {
			data[i * 3 + 0] = image->Data(MEMORYDEVICE_CPU)[i].x;
			data[i * 3 + 1] = image->Data(MEMORYDEVICE_CPU)[i].y;
			data[i * 3 + 2] = image->Data(MEMORYDEVICE_CPU)[i].z;
		}
	}
...
void SaveImageToFile(const ORUtils::Image<short>* image, const char* fileName)
{
	...
	const short *dataSource = image->Data(MEMORYDEVICE_CPU);
	for (size_t i = 0; i < (size_t)image->DataSize(); i++) data[i] = (dataSource[i] << 8) | ((dataSource[i] >> 8) & 255);
...
void SaveImageToFile(const ORUtils::Image<float>* image, const char* fileName)
{
	...
	for (size_t i = 0; i < (size_t)image->DataSize(); i++)
	{
		float localData = image->Data(MEMORYDEVICE_CPU)[i];
...
bool ReadImageFromFile(ORUtils::Image<ORUtils::Vector4<unsigned char> > * image, const char* fileName)
{
	...
	ORUtils::Vector4<unsigned char> *dataPtr = image->Data(MEMORYDEVICE_CPU);

	unsigned char *data;
	if (type != RGBA_8u) data = new unsigned char[xsize*ysize * 3];
	else data = (unsigned char*)image->Data(MEMORYDEVICE_CPU);
...
bool ReadImageFromFile(ORUtils::Image<short> *image, const char *fileName)
{
	...
	{
		for (size_t i = 0; i < size_t(dataSize); ++i)
			image->Data(MEMORYDEVICE_CPU)[i] = (data[i] << 8) | ((data[i] >> 8) & 255);
	}
	else for (size_t i = 0; i < size_t(dataSize); ++i) image->Data(MEMORYDEVICE_CPU)[i] = data[i];
	delete[] data;
