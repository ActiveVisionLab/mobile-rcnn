...
namespace ORUtils {
	...
	public:
		...
		_CPU_AND_GPU_CODE_ inline void setRotation(const T* R) { for (int x = 0; x < 3; x++) for (int y = 0; y < 3; y++)  at(x, y) = R[x * 3 + y]; }
		_CPU_AND_GPU_CODE_ inline void setRotation(const Matrix3_<T>& R) { setRotation(R.m); }
		_CPU_AND_GPU_CODE_ inline void setRT(const T* RT) { for (int x = 0; x < 4; x++) for (int y = 0; y < 3; y++)  at(x, y) = RT[x * 3 + y]; }
		_CPU_AND_GPU_CODE_ inline void setTranslate(const T* t) { for (int y = 0; y < 3; y++) at(3, y) = t[y]; }
		_CPU_AND_GPU_CODE_ inline void setTranslate(const Vector3_<T> &t) {
			at(3, 0) = t.x; at(3, 1) = t.y; at(3, 2) = t.z;
		}
		...
		_CPU_AND_GPU_CODE_ inline Matrix3<T> R() const {
			return Matrix3<T>(this->m00, this->m01, this->m02, this->m10, this->m11, this->m12, this->m20, this->m21, this->m22);
		}
		_CPU_AND_GPU_CODE_ inline Vector3<T> Tr() const {
			return Vector3<T>(this->m30, this->m31, this->m32);
		}
	...
	public:
		...
		_CPU_AND_GPU_CODE_ inline void setZero() { memset(this->m, 0, sizeof(T) * 9); }
		_CPU_AND_GPU_CODE_ inline void setIdentity() { setZero(); this->m00 = this->m11 = this->m22 = 1; }
		_CPU_AND_GPU_CODE_ inline void setScale(T s) { this->m00 = this->m11 = this->m22 = s; }
		_CPU_AND_GPU_CODE_ inline void setScale(const Vector3_<T> &s) { this->m00 = s[0]; this->m11 = s[1]; this->m22 = s[2]; }
		...
		_CPU_AND_GPU_CODE_ inline Matrix3 transpose() const{
			Matrix3 mtrans;
		...
		_CPU_AND_GPU_CODE_ inline T* data() {
			return this->m;
		}
		_CPU_AND_GPU_CODE_ inline const T* data() const {
			return this->m;
		}
		...
		_CPU_AND_GPU_CODE_ Vector3<T> solve(const Vector3<T>& b_) const {
			T d = det();
			if (d == 0)
			...
			Vector3<T> x;
			d = 1 / d;
			x[0] = d*(b_[0]*(this->m11*this->m22 - this->m21*this->m12 ) -
				this->m10 *(b_[1]*this->m22 - this->m21*b_[2]) +
				this->m20 *(b_[1]*this->m12 - this->m11*b_[2]));

			x[1] = d*(this->m00*(b_[1]*this->m22 - this->m21*b_[2]) -
				b_[0]*(this->m01 *this->m22 - this->m21*this->m02 ) +
				this->m20*(this->m01 *b_[2] - b_[1]*this->m02 ));

			x[2] = d*(this->m00 *(this->m11 *b_[2] - b_[1]*this->m12 ) -
				this->m10 *(this->m01 *b_[2] - b_[1]*this->m02 ) +
				b_[0]*(this->m01 *this->m12 - this->m11 *this->m02 ));
