...
namespace ORUtils {
	...
	public:
		...
		_CPU_AND_GPU_CODE_ inline Vector2<short> toShortFloor() const {
			return Vector2<short>((short)floor(this->x), (short)floor(this->y));
		}
		...
		_CPU_AND_GPU_CODE_ T norm() { return sqrt(this->x * this->x + this->y * this->y); }
	...
	public:
		...
		_CPU_AND_GPU_CODE_ inline Matrix3<T> cross() const {
			return Matrix3<T>(0, this->z, -this->y,
				-this->z, 0, this->x,
				this->y, -this->x, 0);
		}
		_CPU_AND_GPU_CODE_ inline Vector3<T> cross(const Vector3<T>& vec) const {
		...
		_CPU_AND_GPU_CODE_ inline void setZero() { memset(this->v, 0, sizeof(T) * 3); }

		_CPU_AND_GPU_CODE_ inline float squaredNorm() const {
		...
		_CPU_AND_GPU_CODE_ inline float norm1() const {
			float norm = FABS(this->x) + FABS(this->y) + FABS(this->z);
			return norm;
		}
		...
		_CPU_AND_GPU_CODE_ inline void setZeros() { memset(this->v, 0, sizeof(T) * 3); }
	...
	public:
		...
		_CPU_AND_GPU_CODE_ inline Vector4<T> normalizeTo1() const {
			float n = norm();
			return Vector4<T>(this->x / n, this->y / n, this->z / n, this->w / n);
		}

		_CPU_AND_GPU_CODE_ inline T norm() const { return sqrt(this->x * this->x + this->y * this->y + this->z*this->z + this->w * this->w); }
	...
	// homogeneous
	template <typename T> _CPU_AND_GPU_CODE_ Vector4<T> convert_homogeneous(const Vector3<T> &in) {
		return Vector4<T>(in.x, in.y, in.z, 1);
	}

	template <typename T> _CPU_AND_GPU_CODE_ Vector3<T> convert_homogeneous(const Vector2<T> &in) {
		return Vector3<T>(in.x, in.y, 1);
	}
