...
class PytorchToLightNet:
    ...
    def writeImageInput(name, size, lntxt):
        lntxt.write("IMAGEINPUT {} size: {} {} {} {} clipRect: 0 0 0 0\n".format(
    ...
    @staticmethod
    def writeImageOutput(name, parent_name, lntxt):
        lntxt.write("IMAGEOUTPUT {} clipRect: 0 0 0 0\n".format(name))
        lntxt.write("\tinput from 1: {}\n".format(parent_name))

        return name

    @staticmethod
    def writeConv(conv, name, parent_name, lntxt, lnweights):
    ...
    @staticmethod
    def writeConvTransposed(conv, name, parent_name, lntxt, lnweights):
        weights = conv.weight.cpu().data.numpy().flatten()
        lnweights.write(struct.pack('f' * len(weights), *weights))
        hasBias = 0
    ...
    @staticmethod
    def writeConvTransposed_forMetal(conv, name, parent_name, lntxt, lnweights):
    ...
    @staticmethod
    def writeConvRelu6(conv, name, parent_name, lntxt, lnweights):
        weights = conv.weight.cpu().data.numpy().flatten()
        lnweights.write(struct.pack('f' * len(weights), *weights))
        hasBias = 0
    ...
    @staticmethod
    def writeBN(bn, name, parent_name, lntxt, lnweights):
    ...
    @staticmethod
    def writeIN(bn, name, parent_name, lntxt, lnweights):
        weights = bn.weight.cpu().data.numpy()
        lnweights.write(struct.pack('f' * len(weights), *weights))
    ...
    @staticmethod
    def writeReLU6(relu, name, parent_name, lntxt, lnweights):
        lntxt.write("RELU6 {}\n".format(name))
        lntxt.write("\tinput from 1: {}\n".format(parent_name))
    ...
    @staticmethod
    def writeReLU(relu, name, parent_name, lntxt, lnweights):
        lntxt.write("RELU {}\n".format(name))
        lntxt.write("\tinput from 1: {}\n".format(parent_name))
    ...
    @staticmethod
    def writeSigmoid(sigmodi, name, parent_name, lntxt, lnweights):
        lntxt.write("SIGMOID {}\n".format(name))
        lntxt.write("\tinput from 1: {}\n".format(parent_name))
    ...
    @staticmethod
    def writeFC_afterReshape(fc, parent_dims, name, parent_name, lntxt, lnweights):
    ...
    @staticmethod
    def writeFC(fc, name, parent_name, lntxt, lnweights):
        #the horrible flip is needed because the MPS reshape doesn't do the same thing as .view(-1)
    ...
    @staticmethod
    def writeRoiAlign(roialign, name, lntxt):
    ...
    @staticmethod
    def writeSum(name, parent_a, parent_b, lntxt):
        lntxt.write("SUM {}\n".format(name))
    ...
    @staticmethod
    def writeReshape(size_out, name, parent_name, lntxt):
    ...
    @staticmethod
    def writeSoftMax(name, parent_name, lntxt):
        lntxt.write("SOFTMAX {}\n".format(name))
        lntxt.write("\tinput from 1: {}\n".format(parent_name))
    ...
    @staticmethod
    def writeMaxPool(maxpool, name, parent_name, lntxt):
        remove_extra = 1
    ...
    @staticmethod
    def writeMaxPoolFromParams(name, kernel_size, stride, padding, ceil_mode, parent_name, lntxt):
        remove_extra = 1
        if ceil_mode:
            remove_extra = 0
    ...
    @staticmethod
    def writeUpSampleNearest(upsample, name, ratio, rect, parent_name, lntxt):
        lntxt.write("UPSAMPLE {} upsample: {} {} kernel: 0 0 crop: 0 rect: {} {} method: nearest\n".format(
    ...
    @staticmethod
    def writeNorm(name, parent_name, lntxt):
        lntxt.write("NORM {}\n".format(name))
        lntxt.write("\tinput from 1: {}\n".format(parent_name))
